  ...
  return (
    <article className="group flex h-full flex-col rounded-3xl bg-white text-center shadow-card ring-1 ring-black/[0.03] transition-all duration-300 hover:-translate-y-1 hover:shadow-lift">
      ...
      <div className="flex flex-1 flex-col p-6 pt-10">
        ...
        <p className="mt-5 font-display text-lg font-bold text-brand-700">
          {course.price.amount} lei/{course.price.unit}
          <span className="block text-sm font-semibold text-ink-muted">
            {course.price.note}
          </span>
        </p>
        <span className="mx-auto mt-4 inline-flex rounded-full bg-sage px-4 py-1.5 text-sm font-semibold text-brand-700">
          {course.grades}
        </span>
      </div>
    </article>
